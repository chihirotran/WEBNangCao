    ...
    if ($errorMsg == "") {
        ...
        elseif($username != "" && $password != ""){
            if ($username != "" && $password != "") {
                ...
                $rows = mysqli_query($conn,"select * from users where  name = $username  and password = $password");
                var_dump($rows);
                // $count = mysqli_num_rows($rows);
                // if ($count==1) {
                //     $is_admin = mysqli_query($conn,"select is_admin from users where  name = $username  and password = $password") ;
                //     if($is_admin==1){
                //         header("Location: themmoi.php");  
                //     }
                //     if($is_admin==0){
